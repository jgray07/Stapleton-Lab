...
cdat <- drop.nullmarkers(cdat)
cscan <- scanonevar(cdat, mean.formula = PlantHeight ~ RFcovar + mean.QTL.add + mean.QTL.dom,
                          var.formula  = ~ RFcovar + var.QTL.add + var.QTL.dom,
                          return.covar.effects = T)
library("dplyr")
...
coutvdf<- data.frame(cscan$result$loc.name[keep],
                     cscan$result$pos[keep],
                     cscan$result$mean.lod[keep],
                     cscan$result$mean.asymp.p[keep],
                     cscan$result$var.lod[keep],
                     cscan$result$var.asymp.p[keep],
                     cscan$result$joint.lod[keep],
                     cscan$result$joint.asymp.p[keep],
                     cscan$result$mef_RFcovar[keep],
                     cscan$result$mse_RFcovar[keep],
                     cscan$result$vef_RFcovar[keep],
                     cscan$result$vse_RFcovar[keep])
coutvdf = cbind(coutvdf,t(csizedf))
colnames(coutvdf) = c("SNP Name",
                      "Position (cM)",
                      "Mean LOD",
                      "Mean P Value",
                      "Variance LOD",
                      ...
                      "Joint LOD",
                      "Joint P Value",
                      "Neighbor Mean Effect Size Est",
                      "Neighbor Mean Effect Size SE",
                      "Neighbor Var Effect Size Est",
                      "Neighbor Var Effect Size SE",
                      "A Mean Est",
                      "A Mean Lower Bound",
                      "A Mean Upper Bound",
...
write.csv(coutvdf, file = "C://Users/Thomas/Documents/GitHub/Stapleton-Lab/vQTL\ Random\ and\ Family/combined/CombinedQTL_LOD,Pvals,EffectSizes-11-7-18.csv",
          row.names = F)
